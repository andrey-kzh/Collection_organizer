...
module.exports = {
  ...
  async returnTotalPages(req, res, next) {
    try {
      ...
      } else { // Кол-во страниц соответствующие поисковому запросу
        const totalItems = await Search.findCatalogCount(search, categories);
        if (totalItems) {
  ...
  async returnSearchResult(req, res, next) {
    try {
      ...
      if ((search.length <= 0) && (categories.length <= 0)) { // Весь каталог, поисковый запрос пуст
        ...
        const result = await Search.findCatalogBySearchString(search, categories, config.catalogPerPage, offset);
        res.status(200).json({ result });
      }
    } catch (e) {
      next(err(e));
